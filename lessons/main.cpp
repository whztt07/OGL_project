/* 
	Данный код является результатом уроков по OpenGL и помогает (в первую и последнюю очередь, мне) понять базовые принципы работы с ним.
...
public:
	...
	bool Init()
	{
		...
		return m_pMesh->LoadMesh("thirdparty/content/phoenix_ugv.md2");
	}
